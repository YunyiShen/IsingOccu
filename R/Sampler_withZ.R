...
IsingOccu.fit.Moller.sampler_withZ = function(X,distM, detmat, detX, Z,mcmc.save = 10000, burn.in = 10 , vars_prior = rep(1,4*ncol(X)+2*ncol(detX[[1]])+9),vars_prop = 2,int_range = "exp",seed = 12345,init,thin.by = 1){
	require(coda)
  ...
  Z_curr = Z
	...
	class(res)="IsingOccu.Moller.withZ"
	return(res)
}
